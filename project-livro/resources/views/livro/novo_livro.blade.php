                    ...
                    <form action="{{ $isEdit ? route('livros.update', $livro->id) : route('livros.guardar') }}" 
                        ...
                        <div class="row g-4 mb-4">
                            <div class="col-md-6">
                                ...
                                <select class="form-select form-select-lg @error('genero') is-invalid @enderror" id="genero" name="genero" required>
                                    @php $generoSelecionado = old('genero', $livro->genero ?? ''); @endphp
                                    <option value="" disabled {{ $generoSelecionado == '' ? 'selected' : '' }}>Selecione uma categoria...</option>
                                    @foreach(['Ficção', 'Romance', 'Fantasia', 'Aventura', 'Terror', 'Biografia'] as $cat)
                                        <option value="{{ $cat }}" {{ $generoSelecionado == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                                    @endforeach
                                </select>
                                @error('genero')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="number" step="0.01" class="form-control form-control-lg @error('preco') is-invalid @enderror" 
                                    id="preco" name="preco" 
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="number" class="form-control form-control-lg @error('estoque') is-invalid @enderror" 
                                    id="estoque" name="estoque" 
                                    value="{{ old('estoque', $livro->estoque ?? '') }}" 
                        ...
                        <div class="d-flex justify-content-end gap-3 pt-3 border-top">
                            <a href="{{ route('home_page') }}" class="btn btn-light border px-4 py-2 fw-semibold fs-6">Cancelar</a>
